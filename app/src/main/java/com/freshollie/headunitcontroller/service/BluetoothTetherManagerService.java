...
public class BluetoothTetherManagerService extends Service {
    private static String TAG = BluetoothTetherManagerService.class.getSimpleName();
    ...
    private static int DELAY_BEFORE_STARTING_CONNECTION = 5000;
    ...
    private synchronized void manageConnections() {
        ...
        if (device != null && bluetoothAdapter.isEnabled()) {
            if (!isWifiConnected()) {
                if (panInterface.getConnectionState(device) == BluetoothProfile.STATE_DISCONNECTED ||
                        panInterface.getConnectionState(device) == BluetoothProfile.STATE_DISCONNECTING) {
        ...
        } else if (!selectedTetherDeviceName.isEmpty() && bluetoothAdapter.isEnabled()) {
    ...
    /**
     * Used to connect the receive a proxy to the bluetooth PAN service and provide
     * an interface for us to connect to a device
     */
    private class BluetoothPANInterface implements BluetoothProfile.ServiceListener {
        ...
        private String PAN_CONNECT_METHOD_NAME = "connect";
        ...
        public void onServiceConnected(int i, BluetoothProfile proxy) {
            ...
            try {
                ...
                Log.v(TAG, "Bluetooth profile service connected");
        ...
        public void onServiceDisconnected(int i) {
            if (bluetoothPan != null) {
                bluetoothPan = null;
                Log.v(TAG, "Bluetooth profile service disconnected");
            }
        }
        ...
        void requestDisconnect(BluetoothDevice device) {
            ...
            Log.v(TAG, "Requesting disconnect");
